...
def evaluate_answer(
    *,
    ...
    faithfulness_value = faithfulness(generated_answer, context_texts)
    relevancy_value = answer_relevancy(question, generated_answer, expected_answer)
    return EvaluationScore(
        top_3_accuracy=top_k_source_accuracy(retrieved_source_ids, expected_source_ids or [], k=3),
        faithfulness=faithfulness_value,
        hallucination_rate=_round(1.0 - faithfulness_value),
        calibration_score=calibration_score(
            confidence=retrieval_confidence(retrieved_contexts),
            correctness=(faithfulness_value + relevancy_value) / 2,
        ),
...
def top_k_source_accuracy(retrieved_source_ids: list[str], expected_source_ids: list[str], k: int = 3) -> float:
    """Return 1 when any expected source appears in the top-k retrieved sources."""

    if not expected_source_ids:
        return 1.0
    top_k = set(retrieved_source_ids[:k])
    return 1.0 if top_k.intersection(set(expected_source_ids)) else 0.0


def retrieval_confidence(retrieved_contexts: list[SearchResult]) -> float:
    """Use the best retrieval score as a bounded confidence estimate."""

    if not retrieved_contexts:
        return 0.0
    score = retrieved_contexts[0].score
    if -1.0 <= score <= 1.0:
        score = (score + 1.0) / 2.0
    return _round(score)


def calibration_score(confidence: float, correctness: float) -> float:
    """Score how closely answer confidence tracks answer correctness."""

    return _round(1.0 - abs(confidence - correctness))
...
def relative_accuracy_lift(current_top_3_accuracy: float, baseline_top_3_accuracy: float) -> float:
    """Compute relative lift in top-3 answer accuracy."""

    if baseline_top_3_accuracy <= 0:
        return 0.0
    return round((current_top_3_accuracy - baseline_top_3_accuracy) / baseline_top_3_accuracy, 4)
