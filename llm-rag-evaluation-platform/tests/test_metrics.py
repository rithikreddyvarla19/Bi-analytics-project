from evaluation.metrics import evaluate_answer, relative_accuracy_lift
...
def test_evaluation_metrics_capture_supported_answer() -> None:
    ...
    assert score.top_3_accuracy == 1.0
    assert score.calibration_score > 0.0
    assert score.context_precision == 1.0
    assert score.context_recall == 1.0


def test_relative_accuracy_lift_matches_resume_metric() -> None:
    assert relative_accuracy_lift(0.762, 0.6) == 0.27
