...
Enterprise Retrieval-Augmented Generation platform with document ingestion, FAISS vector search, source-cited chatbot responses, automated LLM evaluation, MLflow tracking, PostgreSQL feedback storage, and a Streamlit monitoring dashboard.
...
- Generates embeddings with OpenAI, HuggingFace Transformers, or deterministic local hashing.
- Stores and searches vectors with FAISS, with a pure-Python fallback for local tests.
- Serves a source-cited RAG chatbot through FastAPI.
- Supports GPT-style hosted models and open-source HuggingFace models.
- Evaluates faithfulness, hallucination rate, context precision, context recall, and answer relevancy.
- Validates ground-truth datasets before evaluation runs.
- Compares prompt variants and tracks experiments in MLflow.
- Displays accuracy metrics, hallucination trends, latency, feedback, and prompt comparisons in Streamlit.
...
evaluation/       Ground-truth validation, metrics, prompt experiments
...
curl -X POST http://localhost:8000/chat ^
  ...
  -d "{\"question\":\"How often is hallucination monitoring performed?\",\"top_k\":3}"
```
